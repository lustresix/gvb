...
const (
	// recommendedHomeDir 定义放置 blog 服务配置的默认目录.
	recommendedHomeDir = ".goblogv2"

	// defaultConfigName 指定了 blog 服务的默认配置文件名.
	defaultConfigName = "setting"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 GBV2，如果是 gbv2，将自动转变为大写。
	viper.SetEnvPrefix("GBV2")
